...
// Works both on HTTPS (GitHub/Cloudflare Pages) and in local previews where
// crypto.randomUUID() may not be available.
const id=()=>"watch-"+Date.now().toString(36)+"-"+Math.random().toString(36).slice(2)+"-"+Math.random().toString(36).slice(2);
const initial={categories:["Trabajo","Daily","Vestir","Casual"],watches:[
{id:id(),name:"Seiko 5 6119",brand:"Seiko",reference:"",photo:"",categories:["Daily","Casual","Vestir"],uses:0},
{id:id(),name:"Cauny Prima",brand:"Cauny",reference:"",photo:"",categories:["Vestir","Daily"],uses:0},
{id:id(),name:"Camy Montego 7649",brand:"Camy",reference:"7649",photo:"",categories:["Daily","Casual"],uses:0},
{id:id(),name:"PUBULUS Batman",brand:"PUBULUS",reference:"",photo:"",categories:["Trabajo","Daily","Casual"],uses:0},
{id:id(),name:"G-Shock GA-2100 Manga Azul",brand:"Casio",reference:"GA-2100",photo:"",categories:["Trabajo","Daily","Casual"],uses:0},
{id:id(),name:"Casio MRW-200H",brand:"Casio",reference:"",photo:"",categories:["Trabajo","Daily","Casual"],uses:0},
{id:id(),name:"Timex Ocean",brand:"Timex",reference:"",photo:"",categories:["Daily","Casual"],uses:0},
{id:id(),name:"Festina F20695/4",brand:"Festina",reference:"F20695/4",photo:"",categories:["Vestir","Daily","Casual"],uses:0},
{id:id(),name:"Festina F20709/1",brand:"Festina",reference:"F20709/1",photo:"",categories:["Vestir","Daily","Casual"],uses:0},
{id:id(),name:"Homage JLC Reverso",brand:"AliExpress homage",reference:"",photo:"",categories:["Vestir","Casual"],uses:0}]};
function load(){
try{
  let x=JSON.parse(localStorage.getItem(KEY));
  if(!x){ const old=JSON.parse(localStorage.getItem("watch-roulette-data-v1")); x=old || JSON.parse(JSON.stringify(initial)); }
...
let data=load(); const save=()=>{try{localStorage.setItem(KEY,JSON.stringify(data));}catch(e){console.warn("No se pudo guardar localmente",e);}};
...
let deferred=null;addEventListener("beforeinstallprompt",e=>{e.preventDefault();deferred=e;const b=document.querySelector("#installBtn");if(b)b.classList.remove("hidden")});const ib=document.querySelector("#installBtn");if(ib)ib.onclick=()=>{if(deferred){deferred.prompt();deferred=null}};home();
